...
<html>
	<head>	
		...
		<title><g:message code="etech.report.emp.by.project.by.activity.title.label" default="Employee by Project/Activity Report"/></title>
		<content tag="pageName">employeeByProjectByActivityPage</content>
	</head>
	<body>			
		<div class="navigation"> <h3> <g:message code="etech.report.emp.by.project.by.activity.title.label" default="Employee by Project/Activity Report" /> </h3> </div>				
     	<div class="white-bg">     	
			<div class="title ">			
			...
			<div class="col-xs-12">							 
					...
					<table class="table without-border">	
						<g:if test="${projectActualLaborDtlInstanceList?.size() > 0}">
							<g:if test="${params?.includeDetail == 'true' }">	
								<g:render template="employeeByProjectByActivity/ajaxProjectByActivityDetailList"></g:render>
							</g:if>	
							<g:else>
								<g:render template="employeeByProjectByActivity/ajaxProjectByActivityList"></g:render>
							</g:else>
							<!-- Report Total End -->		
				...
				<g:if test="${projectActualLaborDtlInstanceList?.size() > 0}">
					<g:form controller="report" action = "exportReport" method="post">
						...
						<g:hiddenField name="reportName" value="${org.solcorp.etech.Constants.PROJECT_BY_ACTIVITY_REPORT_NAME}"/>
						 <a class="btn btn-default btn-sm" onclick="exportExcel(this)"> <i class="fa fa-file-excel-o" ></i> Export Excel </a>
					</g:form>
					<br/>	
				</g:if>
				<!-- Export Report Start -->					
				<g:render template="/common/returnLink" model="['returnAction': 'employeeByProjectByActivityReport', 'returnController': 'report', 'messageCode':'etech.report.return.filter', 'defaultMessage': 'Return to Report Filter']"></g:render>
			</div>			

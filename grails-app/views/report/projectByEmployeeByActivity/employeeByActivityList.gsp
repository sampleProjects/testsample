...
<html>
	<head>	
		...
		<title><g:message code="etech.report.project.by.emp.activity.title.label" default="Project by Employee/Activity Report"/></title>
		<content tag="pageName">employeeByActivityPage</content>
	</head>	
	<body>			
		<div class="navigation"> <h3> <g:message code="etech.report.project.by.emp.activity.title.label" default="Project by Employee/Activity Report" /> </h3> </div>				
     	<div class="white-bg">     	
			<div class="title ">			
			...
			<div class="col-xs-12">							 
				<div class="table-responsive">	
					<g:render template="projectByEmployeeByActivity/ajaxEmployeeByActivityList"></g:render>	
				</div>	
				...
				<g:if test="${projectActualLaborDtlInstanceList?.size() > 0}">
					<g:form controller="report"  action = "exportReport" method="post">
						...
						<g:hiddenField name="reportName" value="${org.solcorp.etech.Constants.EMPLOYEE_BY_ACTIVITY_REPORT_NAME}"/>
						<a  class="btn btn-default btn-sm" onclick="exportExcel(this)"><i class="fa fa-file-excel-o" ></i> Export Excel</a>
					</g:form>						 
					<br/>
				</g:if>					
				<g:render template="/common/returnLink" model="['returnAction': 'employeeByActivityReport', 'returnController': 'report', 'messageCode':'etech.report.return.filter', 'defaultMessage': 'Return to Report Filter']"></g:render>
			</div>

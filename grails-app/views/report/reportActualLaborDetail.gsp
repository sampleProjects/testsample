...
<body>
	...
	<div class="navigation">	
		<h3> <g:message code="etech.project.actual.labor.employee.label" default="Actual Labor By Employee for" /> ${projectInstance?.name} ( ${projectInstance?.code} )</h3>		
	</div>	
	<div class="col-sm-12" role="main">			
		<div class="white-bg">		
			<div class="title"><h2> <g:message code="etech.project.actual.labor.list.label" default="Actual Labor List" />	</h2></div>
          	<div>          	
				<div class="col-xs-12">				
						...
						<g:if test="${employeeDetailDTOList?.size() > 0}">							
							<g:each in="${employeeDetailDTOList}" status="i" var="employeeDetailDTOInstance">							
								<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
									<td> <g:link action="getAllActualLaborActivityDetail" controller="report" params="[projId: projectInstance?.id, empId: employeeDetailDTOInstance?.employyeeId,costOption: params?.costOption]">${employeeDetailDTOInstance?.employeeName}</g:link></td>
				...
				<g:render template="/common/returnLink" model="['returnAction': 'getProjectPerformanceDetail', 'returnController': 'report', 'messageCode':'etech.project.performance.return.screen', 'defaultMessage': 'Return to Project Performance Screen', 'linkId': projectInstance?.id,'linkParam': ['costOption': params?.costOption]]"></g:render>				
			</div>		
			</div>						
		</div>
	</div>

						...
						<g:if test="${userSessionList?.size() > 0}">
							<g:each in="${userSessionList}" status="i" var="userSessionInstance">
								<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
									...
									<td>${userSessionInstance?.user?.username}</td>
									<td>${userSessionInstance?.user?.employee?.firstName}</td>
									<td>${userSessionInstance?.user?.employee?.lastName}</td>
									<td>${DateFormatUtils.getStringFromDate(userSessionInstance?.startDate)}</td>									

...
<g:render template="/common/paginationCombo" model="['totalRecord':employeeInstanceCount, 'searchAction': 'getEmployeeListWithoutUser', 'searchController': 'user', 'divIdToUpdate': 'employeeListGridDiv']"></g:render>
	...
	<div id="ajaxListView" class="table-responsive height-scroll">	
  		...
  		<table class="table table-striped">	
			...
			<g:if test="${empInstanceList?.size() > 0}">
	    		<g:each in="${empInstanceList}" status="i" var="employeeInstance">
					<tr>			
						<td><div style="cursor: pointer;" onclick="setEmployeeCode('${employeeInstance?.id}','${employeeInstance?.code}')" >${fieldValue(bean: employeeInstance, field: "code")}</div></td>
	...
	<g:render template="/common/ajaxSortingAndPaging" model="['searchAction': 'getEmployeeListWithoutUser', 'searchController': 'user', 'divIdToUpdate': 'employeeListGridDiv', 'searchParams': 'true']" />			
